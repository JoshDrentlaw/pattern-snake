...
export default function (entities, { touches, dispatch }) {
        ...
        pellets = pellets.filter(p => {
            if (
                ...
                tail.push({
                    name: 'tail',
                    position: [
                        ['up', 'left'].includes(lastTail.direction) ? lastTail[0] - (SIZE * 2) : lastTail[0],
                        ['down', 'right'].includes(lastTail.direction) ? lastTail[1] - (SIZE * 2) : lastTail[1]
                    ],
                    color: '#666',
                    renderer: <Tail />
                })
    ...
    if (touches.length > 0) {
        ...
        touches.filter(t => t.type === 'move').forEach(({ delta: { pageX, pageY } }) => {
    ...
    if (head.update === 0) {
        ...
        tail.forEach(t => {
            if (
                t.position[0] === head.position[0] &&
                t.position[1] === head.position[1]
            ) {
                dispatch({msg: 'game-over', score: patternDisplay.score})
            }
        })
    ...
    function randomLocation() {
        return [
            Math.floor(Math.random() * (right - 75)) + 75,
            Math.floor(Math.random() * (bottom - 75)) + 75
        ]
    }
    ...
    function getLastTailDirection(oldPos, tail) {
        const deltaX = oldPos[0] + tail[0],
            deltaY = oldPos[1] + tail[1]
